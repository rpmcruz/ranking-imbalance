...
if __name__ == '__main__':
    ...
    names = []
    models = []
    for h in np.linspace(10, 40, 5):
        h = int(h)
        names.append('NeuralNet %d' % h)
        models.append(NeuralNet(h, False))
        names.append('NeuralNet b %d' % h)
        models.append(NeuralNet(h, True))
        names.append('RankNet %d' % h)
        models.append(RankNet(h))
    test.test(names, models, 5)
